...
b <- expand_grid(
        ...
        avars = list(c("bio1"),
                     c("bio1", "wspeed")),
        topo_vars = list(c("northness", "eastness"),
                         c("northness", "eastness", "tpi"),
                         c("northness", "eastness", "tpis"),
                         c("northness", "eastness", "windward"),
                         c("northness", "eastness", "tpi", "windward"),
                         c("northness", "eastness", "tpis", "windward")),
        subplots = list(c(1:4),
                        1))

# wspeed and windward should be used together or not at all
b$wind <- NA
for(i in 1:nrow(b)){
        b$wind[i] = "wspeed" %in% b$avars[[i]] + 
                "windward" %in% b$topo_vars[[i]]
}
b <- b %>% filter(wind != 1) %>% select(-wind)

# file paths for binned data
b <- b %>% mutate(file_path = paste0("data/derived/binned/param_", 1:nrow(.), ".rds"))

# save parameter metadata
saveRDS(b, "data/derived/binned/metadata.rds")
