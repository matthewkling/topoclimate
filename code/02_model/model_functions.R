...
# build basis splines
make_splines <- function(x, y, z = NULL, 
                         xbounds = NULL, ybounds = NULL, zbounds = NULL,
                         knots = 2, degree = 3){
        ...
        if(is.null(ybounds)) ybounds <- range(y)
        if(is.null(zbounds) & !is.null(z)) zbounds <- range(z)
        B1 <- bs(x, knots = k, Boundary.knots = xbounds, degree=degree, intercept = TRUE)
        B2 <- bs(y, knots = k, Boundary.knots = ybounds, degree=degree, intercept = TRUE)
        if(!is.null(z)) B3 <- bs(z, knots = k, Boundary.knots = zbounds, degree=degree, intercept = TRUE)
        B <- rep(1, nrow(B1))
        for(i in 1:ncol(B1)){
                for(j in 1:ncol(B2)){
                        if(is.null(z)){
...
fit_pspline_model <- function(data_file = metadata[1,]$data_file,
                              ...
                              hmc = T, # whether to run hmc following pml (which is always run)
        ...
        if(!dir.exists(hmc_dir) & hmc) dir.create(hmc_dir)
        ...
        if(hmc & ((!file.exists(hmc_file)) | hmc_redo)){
                ...
                fit$summary() %>% 
                        write_csv(paste0(hmc_dir, "/fit_summary.csv"))
